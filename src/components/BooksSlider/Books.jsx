...
const Books = () => {
  ...
  const handleUpdate = () => {
    ...
    if(isNaN(id)){
      ...
      fetch(`http://192.168.1.3:4800/updateBook?id=${id}&imgUrl=${imgUrl}&title=${title}&description=${description}&author=${author}&price=${price}&rating=${rating}&type=${type}`, {
        method: 'GET',
        headers: {
  ...
  const handleDelete = () => {
    ...
    if(isNaN(id)){
      ...
      fetch(`http://192.168.1.3:4800/deleteBook?id=${id}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const getBooks = async (type) => {
    ...
    await  fetch(`http://192.168.1.3:4800/getBooks?type=${type}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json'
        },
